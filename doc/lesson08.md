...
Поэтому для сравнения списков еды создадим отдельный `TO_MATCHER`, с помощью статического фактори-метода `usingEqualsComparator(MealTo.class)`:
```
...
Для того чтобы для тестов создать объекты `MealTo` используем утильный метод `MealsUtil#createTo`, изменив у него модификатор доступа на *public*.
...
в скобках для которой перечисляем поля объекта json, соответствующие аргументам конструктора.
...
В нашем случае параметры фильтрации еды - дата и время - по REST приходят в виде строки и мы можем добавить свой конвертор или форматтер, чтобы он автоматически приводил их к нужному нам типу.
...
Сделаем собственные форматтеры для преобразования строки в дату и время `DateTimeFormatters`, добавим в `spring-mvc.xml` бин `conversionService` c перечнем наших форматтеров и сделаем на него ссылку:    
```
...
интерфейс `Formatter<Целевой тип>` и переопределять его методы `#parse` и `#print`. Теперь мы можем убрать аннотации `@DateTimeFormat` из аргументов `MealRestController#getBetween`. Spring будет
искать в своем контексте подходящие форматеры или конвертеры, которые смогли бы преобразовать параметр-строку в объект соответствующего типа, объявленный в методе контроллера и найдет наши кастомные форматеры, зарегистрированные в его контексте.  
Для этого нового метода теперь создадим несколько тестов - с различным набором параметров (в том числе и с пустыми параметрами).
...
Также популярным средством тестирования REST является *Postman* и в IDEA: *Tools->HTTP Client*.
...
Таким образом, мы имеем дело с *BiDirectional* и циклической зависимостью. При сериализации через Jackson у нас возникнут проблемы, так как он перейдет в 
бесконечный цикл при переходе по ссылкам сущностей друг на друга.  
Возможно следующее разрешение циклических зависимостей: 

- Над полем `Meal.user` добавить аннотацию `@JsonBackReference`, теперь для еды это поле не будет сериализовываться в json;
- Над коллекцией `User.meals` добавить аннотацию `@JsonManagedReference`, поле будет сериализовываться.
...
Добавим в наш проект, в  `pom.xml` дополнительные зависимости на библиотеки JavaScript и css:  
...
FrontEnd нашего приложения будет строиться на основе фреймворка BootStrap. 
> В новой версии BootStrap 5 из зависимостей исключена библиотека jQuery и весь необходимый BootStrap функционал делается на простом JavaScript. Однако JQuery нам нужна для *datatables* и плагинов, поэтому не стал переходить на 5-ю версию.
...
- на странице `index.jsp` форму выбора пользователя поместим в *jumbotron* класс BootStrap - крупный выносной элемент с большим текстом и большими отступами  
...
Оформляем таблицу пользователей с помощью js/css библиотеки `Datatable`. Таблица должна иметь id (в нашем случае "datatable"), чтобы к ней можно было обращаться.  
...
``` 
преобразует HTML элемент c *id=datatable* в javaScript объект с помощью метода `DataTable` библиотеки *Datatable*.
...
Внизу конфигурации добавляется сортировки таблицы по первому столбцу.
...
- Метод `add` вызывается из `users.jsp` по нажатию на кнопку "Добавить":  `onclick="add()"`. В нем
  - обнуляются все `input` поля формы `detailsForm`:  `$("#detailsForm").find(":input").val("")`  
...
- В `save` средствами jQuery сериализуем форму `id=detailsForm` в JSON объект и в POST отдаем эти данные в контроллер. После успешного выполнения запроса закрываем модальное окно и обновляем таблицу. 
...
Поэтому определение css стилей обычно размещают в самом начале HTML документа, а JavaScript - в конце документа, чтобы пользователь видел отрисованную страницу не дожидаясь загрузки скриптов.
...
атрибута *async* используется атрибут *defer*. У нас загрузка скриптов сделана через *defer* и вынесена в `headTag` фрагмент -  скрипты будут загружаться в
фоне и это не будет тормозить загрузку страницы.
Для корректного отображения Datatable в Bootstrap добавлены дополнительные `dataTables.bootstrap4` стили и js скрипты.  

Для удобства дебага js скрипты вынесены в отдельные файлы и сделал небольшой рефакторинг.
</details>
...
Если AJAX запрос завершиться неуспешно (например произойдет исключение на сервере), то помимо действия *succes/done* в AJAX запросе мы можем определить и действие *fail*, которое выполнится при статусе ответа, отличном от 2xx.
