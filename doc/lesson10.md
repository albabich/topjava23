...
Решим эту проблему через [JSON Views](http://www.baeldung.com/jackson-json-view-annotation) - с помощью вью-аннотаций в контроллерах будет возвращаться даты в разных форматах: поле `dateTime` в формате ISO будет
...
По умолчанию в `JsonObjectMapper` включим REST вью: `#setConfig(getSerializationConfig().withView(View.JsonREST.class))`. Оно будет применяться везде в контроллерах, где вью  не указано явно.     
Jackson будет:
...
- если в контроллере вью аннотаций нет - использовать сконфигурированный по умолчанию `View.JsonREST`;
...
2. При создании контекста аннотация `<security:global-method-security...` говорит про то, что все бины текущего контекста, у которых есть аннотации авторизации (`@PreAuthorize/ @Secured/ ..`) будут
   проксироваться с проверкой авторизации. Контексты родителей и детей создаются сами по себе и аннотации проксирования у них собственные.
...
Аннотация `@RestControllerAdvice(annotations = RestController.class)` означает, что будут обрабатываться только эксепшены от контроллеров `@RestController`.
...
Для POST запросов из JSP форм, вместо добавления в форму поля CSRF токена, мы можем использовать тег Spring `form:form` - Spring добавит это поле сам.
